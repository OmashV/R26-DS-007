...
First classify the RESPONSE TYPE:

- "answer_attempt":
  The student makes an assessable mathematical claim, gives an answer,
  proposes a method/step, or states a mathematical conclusion. Uncertain
  wording does not make it a non-answer when an assessable claim is present.
  Examples: "Maybe 15", "I think it is 10", "0.2 times 50 is 10".

- "non_answer":
  The student does not make an assessable mathematical claim. This includes
  explicit lack of understanding/knowledge, requests for explanation,
  clarification or hints, abstention, or purely conversational/meta responses.
  Examples: "I don't understand", "I don't know", "Can you explain that?",
  "What does percent mean?", "Can I have a hint?"

Then classify correctness.
...
- If response_type is "non_answer", correctness MUST be "unknown".
- "incorrect" is reserved for an assessable answer/claim that is actually wrong.
- Use "partial" only when an answer attempt contains meaningful correct progress
  but is incomplete or also contains a substantive error.
- Use "unknown" when the response is a non-answer or when the supplied
  problem/reference/rubric is insufficient to judge an answer attempt reliably.

Do NOT treat uncertainty, help-seeking, clarification requests, tone,
reasoning quality, or tutor quality themselves as correctness.
...
Return JSON only:
{{
  "response_type": "answer_attempt|non_answer",
...
""".strip()
        ...
        if response_type not in {"answer_attempt", "non_answer"}:
            return EvaluationVerdict(
                correctness=Correctness.UNKNOWN,
                confidence=0.0,
                source="gemini_structured_fallback_invalid_response_type",
            )

        if response_type == "non_answer":
            return EvaluationVerdict(
                correctness=Correctness.UNKNOWN,
                confidence=0.0,
                source="gemini_structured_non_answer",
            )
